...
describe("workbench image artifact apis", () => {
  it("returns image-build manifest json with attachment headers", async () => {
    const request = new NextRequest("https://freshcrate.ai/api/workbench/image-build?bundle=automation-node&mode=headless&channel=beta&image=aws-ami-builder&download=1");
    const response = GET_IMAGE_BUILD(request);
    expect(response.status).toBe(200);
    expect(response.headers.get("content-type")).toContain("application/json");
    expect(response.headers.get("content-disposition")).toContain("freshcrate-image-build-automation-node-headless-beta-aws-ami-builder.json");
